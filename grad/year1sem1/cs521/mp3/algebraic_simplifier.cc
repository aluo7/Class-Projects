...
using namespace xla::protobuf_util;
...
absl::Status AlgebraicSimplifierVisitor::HandleAdd(HloInstruction* add) {
  ...
  // 2 ->
  // LHS: add(divide(x, broadcast(y)), divide(z, broadcast(y))) => pre RHS: divide(add(x, z), broadcast(y))
  // pre: y scalar R0 F32; x.shape()==z.shape(); element_type=F32
  VLOG(10) << "trying transform [(A / C) + (B / C) => (A + B) / C]";
  if (Match(add,
          m::Add(
              m::Divide(m::Op(&a), m::Broadcast(m::Op(&c1).WithShape(m::Shape().IsScalar()))),
              m::Divide(m::Op(&b), m::Broadcast(m::Op(&c2).WithShape(m::Shape().IsScalar()))))
      ) && c1 == c2
  ) {
      return ReplaceWithNewInstruction(
          add, HloInstruction::CreateBinary(
              ...
              add->mutable_operand(0)->mutable_operand(1)
          )
      );
  }
...
absl::Status AlgebraicSimplifierVisitor::HandleSubtract(HloInstruction* sub) {
  ...
  // 3.2 ->
  // LHS: subtract(power(add(A,B),const(2.0)), multiply(add(A,B),C)) => pre RHS: multiply(add(A,B), subtract(add(A,B),C))
  // pre: elementwise ops, broadcast-compatible; const(2.0) R0 F32; A.shape()==B.shape()==C.shape(); element_type=F32.
  VLOG(10) << "trying transform [Square(A + B) - (A + B) * C => (A + B) * (A + B - C)]";
  HloInstruction *add_lhs_pow, *add_rhs_pow, *add_lhs_mul, *add_rhs_mul, *mul_rhs, *pow_exp;
  if (Match(sub, m::Subtract(m::Power(
                                  m::AddAnyOrder(m::Op(&add_lhs_pow), m::Op(&add_rhs_pow)),
                                  m::ConstantEffectiveScalar(&pow_exp)),
                              m::MultiplyAnyOrder(m::AddAnyOrder(m::Op(&add_lhs_mul), m::Op(&add_rhs_mul)),
                                                  m::Op(&mul_rhs))))) {

    if (IsAll(pow_exp, 2.0f) &&  // confirm exponent is 2.0 for square
        ((add_lhs_pow == add_lhs_mul && add_rhs_pow == add_rhs_mul) ||
        (add_lhs_pow == add_rhs_mul && add_rhs_pow == add_lhs_mul))) {  // adds must match (commutative)

      // recompute the common add (A + B) once
      HloInstruction* common_add = sub->parent()->AddInstruction(
          HloInstruction::CreateBinary(sub->shape(), HloOpcode::kAdd, add_lhs_pow, add_rhs_pow));
        
      // compute (A + B - C)
      HloInstruction* new_sub = sub->parent()->AddInstruction(
          HloInstruction::CreateBinary(sub->shape(), HloOpcode::kSubtract, common_add, mul_rhs));

      // replace with multiply of (A + B - C) and (A + B)
      return ReplaceWithNewInstruction(
          sub, HloInstruction::CreateBinary(sub->shape(), HloOpcode::kMultiply, new_sub, common_add));
    }
  }

  // alternative matching for when power is expanded to multiply: (A + B) * (A + B) - (A + B) * C
  else if (Match(sub, m::Subtract(
                          m::MultiplyAnyOrder(
                              m::AddAnyOrder(m::Op(&add_lhs_pow), m::Op(&add_rhs_pow)),
                              m::AddAnyOrder(m::Op(&add_lhs_mul), m::Op(&add_rhs_mul))),
                          m::MultiplyAnyOrder(
                              m::AddAnyOrder(m::Op(&add_lhs_pow), m::Op(&add_rhs_pow)),
                              m::Op(&mul_rhs))))) {
    if ((add_lhs_pow == add_lhs_mul && add_rhs_pow == add_rhs_mul) ||
        (add_lhs_pow == add_rhs_mul && add_rhs_pow == add_lhs_mul)) {  // adds must match
  
      // recompute the common add (A + B)
      HloInstruction* common_add = sub->parent()->AddInstruction(
          HloInstruction::CreateBinary(sub->shape(), HloOpcode::kAdd, add_lhs_pow, add_rhs_pow));
      ...
      // replace with multiply of (A + B) and (A + B - C)
      return ReplaceWithNewInstruction(
          sub, HloInstruction::CreateBinary(sub->shape(), HloOpcode::kMultiply, common_add, new_sub));
    }
  }
...
absl::Status AlgebraicSimplifierVisitor::HandleDot(HloInstruction* dot) {
  ...
  // 3.1
  // LHS: dot(A,B,dnums,prec); dot(A,C,dnums,prec) => pre RHS: concat=concatenate(B,C,dim=non_contract_rhs);
  //       combined=dot(A,concat,dnums,prec); x=slice(combined,limit[output_dim]=B.dim(non_contract_rhs)); y=slice(combined,start[output_dim]=B.dim(non_contract_rhs))
  // pre: single contracting dim; B.shape()==C.shape() except concat_dim; A has 2 dots; no batch; element_type=F32.
  VLOG(10) << "trying transform [dot(A, B), dot(A, C) => X, Y where [X Y] = dot(A, concat(B, C))";
  HloInstruction *shared_lhs, *rhs1, *rhs2, *other_dot;
  if (Match(dot, m::Dot(m::Op(&shared_lhs), m::Op(&rhs1)))) {  // match current dot as dot(shared_lhs, rhs1)

    //check if shared_lhs has exactly two users (potential parallel dots)
    if (shared_lhs->user_count() == 2 && shared_lhs->opcode() == rhs1->opcode() &&
        rhs1->opcode() != HloOpcode::kConcatenate && rhs1->opcode() != HloOpcode::kSlice) {
      other_dot = nullptr;
      rhs2 = nullptr;
      HloInstruction* temp_lhs = nullptr; // Temporary variable for matching

      // iterate users in shared_lhs to find the other dot with matching structure
      for (HloInstruction* user : shared_lhs->users()) {
        if (user != dot && user->opcode() == HloOpcode::kDot) {
          if (Match(user, m::Dot(m::Op(&temp_lhs), m::Op(&rhs2))) && temp_lhs == shared_lhs) {  // confirm shared LHS
            other_dot = user;
            break;
          }
        }
      ...
      if (other_dot && rhs2) {
        ...
        if (rhs1->opcode() == HloOpcode::kConcatenate || rhs2->opcode() == HloOpcode::kConcatenate) {
          return absl::OkStatus();
        }

        // extract contracting dimension (assume single)
        const DotDimensionNumbers& dnums = dot->dot_dimension_numbers();
        // ### FIX: Use RHS contracting dim to find RHS non-contracting dim ###
        int64_t contracting_dim_rhs = dnums.rhs_contracting_dimensions(0);
        int64_t rhs_non_contract_dim = (rhs1->shape().rank() == 1) ? 0 : (contracting_dim_rhs == 0 ? 1 : 0);
        // ### END FIX ###
        int64_t size_rhs1 = rhs1->shape().dimensions(rhs_non_contract_dim);
        int64_t size_rhs2 = rhs2->shape().dimensions(rhs_non_contract_dim);

        std::vector<HloInstruction*> concat_operands = {rhs1, rhs2};
        std::vector<const Shape*> operand_shapes = {&rhs1->shape(), &rhs2->shape()};

        TF_ASSIGN_OR_RETURN(
            Shape concat_shape,
            ShapeInference::InferConcatOpShape(operand_shapes, rhs_non_contract_dim));

        // create concat instruction along non-contracting dim
        HloInstruction* concat = dot->parent()->AddInstruction(
            HloInstruction::CreateConcatenate(concat_shape, concat_operands, rhs_non_contract_dim));
            
        TF_ASSIGN_OR_RETURN(
            Shape combined_dot_shape,
            ShapeInference::InferDotOpShape(
                shared_lhs->shape(), concat->shape(), dnums, dot->shape().element_type())); // <-- Changed std::nullopt

        // fused dot with shared LHS and concatenated RHS
        HloInstruction* combined_dot = dot->parent()->AddInstruction(
            HloInstruction::CreateDot(combined_dot_shape, shared_lhs, concat, dnums, dot->precision_config()));
            
        // output dim for splitting
        int64_t output_concat_dim = combined_dot_shape.rank() - 1;
        
        std::vector<int64_t> start_indices(combined_dot_shape.rank(), 0);
        std::vector<int64_t> limit_indices(combined_dot_shape.dimensions().begin(),
                                           combined_dot_shape.dimensions().end());
        limit_indices[output_concat_dim] = size_rhs1;
        std::vector<int64_t> strides(combined_dot_shape.rank(), 1);
        HloInstruction* x_slice = dot->parent()->AddInstruction(
        ...
        std::vector<int64_t> start_indices_y(combined_dot_shape.rank(), 0);
        start_indices_y[output_concat_dim] = size_rhs1;
        std::vector<int64_t> limit_indices_y(combined_dot_shape.dimensions().begin(),
                                             combined_dot_shape.dimensions().end());
        limit_indices_y[output_concat_dim] = size_rhs1 + size_rhs2;
        HloInstruction* y_slice = dot->parent()->AddInstruction(
            HloInstruction::CreateSlice(
                other_dot->shape(),
                combined_dot,
                start_indices_y,
        ...
        TF_RETURN_IF_ERROR(other_dot->ReplaceAllUsesWith(y_slice));
        return ReplaceInstruction(dot, x_slice);
      }
    }
  }
...
absl::Status AlgebraicSimplifierVisitor::HandleMultiply(
  ...
  {
    // 3.3 -> 
    // LHS: multiply(divide(const(1.0),multiply(A,B)), multiply(C,divide(const(1.0),multiply(A,B)))) => 
    //        pre RHS: multiply(power(divide(const(1.0),multiply(A,B)),const(2.0)),C)
    // pre: elementwise ops, broadcast-compatible; consts R0 F32; A.shape()==B.shape()==C.shape(); element_type=F32.
    VLOG(10) << "trying transform [Recip(Mul(A, B)) * (Recip(Mul(A, B)) * C) => Square(Recip(Mul(A, B))) * C]";
    HloInstruction *mul_lhs_outer, *temp_inner_mul, *mul_rhs;

    if (Match(multiply->operand(0), m::Power(m::Divide(), m::ConstantEffectiveScalar(2.0f)))) {
      return absl::OkStatus();
    }
    
    if (Match(multiply,
              m::MultiplyAnyOrder(
                  m::Divide( // Recip(Mul(A,B))
                      m::ConstantEffectiveScalar(1.0f),
                      // bind the instruction for the inner multiply
                      m::MultiplyAnyOrder(&mul_lhs_outer, m::Op(), m::Op())
                  ),
                  m::MultiplyAnyOrder( // (C * Recip(Mul(A,B)))
                      m::Op(&mul_rhs),
                      m::Divide(
                          m::ConstantEffectiveScalar(1.0f),
                          // bind the *other* inner multiply instruction
                          m::MultiplyAnyOrder(&temp_inner_mul, m::Op(), m::Op())
                      )
                  )
              ))) {

      if (mul_lhs_outer == temp_inner_mul) { 
            
        HloInstruction* one_scalar = multiply->parent()->AddInstruction(
            HloInstruction::CreateConstant(LiteralUtil::CreateR0<float>(1.0f)));
        HloInstruction* one_broadcast = multiply->parent()->AddInstruction(
            HloInstruction::CreateBroadcast(multiply->shape(), one_scalar, {}));
        
        HloInstruction* recip = multiply->parent()->AddInstruction(
            HloInstruction::CreateBinary(multiply->shape(), HloOpcode::kDivide, one_broadcast, mul_lhs_outer));
            
        HloInstruction* two_scalar = multiply->parent()->AddInstruction(
            HloInstruction::CreateConstant(LiteralUtil::CreateR0<float>(2.0f)));
        HloInstruction* two_broadcast = multiply->parent()->AddInstruction(
            HloInstruction::CreateBroadcast(multiply->shape(), two_scalar, {}));
        HloInstruction* recip_sq = multiply->parent()->AddInstruction(
            HloInstruction::CreateBinary(multiply->shape(), HloOpcode::kPower, recip, two_broadcast));

        // replace with multiply of squared reciprocal and C
        return ReplaceWithNewInstruction(
            multiply,
            HloInstruction::CreateBinary(multiply->shape(), HloOpcode::kMultiply, recip_sq, mul_rhs));
      }
    }
  }
  ...
  {
    // 3.4 -> 
    // LHS: multiply(multiply(A,broadcast(reduce(B,const(0.0),dims=D,comp=add))), multiply(broadcast(reduce(B,const(0.0),dims=D,comp=add)),C)) => 
    //        pre RHS: multiply(multiply(A,power(broadcast(reduce(B,const(0.0),dims=D,comp=add)),const(2.0))),C)
    // pre: reduces match dims=D, init const(0.0) R0 F32, comp root=add; broadcast-compatible; A.shape()==C.shape(); element_type=F32.
    VLOG(10) << "trying transform [(A * ReduceSum(B)) * (ReduceSum(B) * C) => A * Square(ReduceSum(B)) * C]: " << multiply->ToString();
    HloInstruction *mul_lhs, *reduce_input1, *reduce_input2, *mul_rhs, *reduce_op1, *reduce_op2, *reduce_bcast1, *reduce_bcast2;
    if (Match(multiply,
              m::MultiplyAnyOrder(
                  m::MultiplyAnyOrder(
                      m::Op(&mul_lhs),
                      m::Broadcast(
                          &reduce_bcast1,
                          m::Reduce(&reduce_op1, m::Op(&reduce_input1), m::ConstantEffectiveScalar(0.0f)))),
                  m::MultiplyAnyOrder(
                      m::Broadcast(
                          &reduce_bcast2,
                          m::Reduce(&reduce_op2, m::Op(&reduce_input2), m::ConstantEffectiveScalar(0.0f))),
                      m::Op(&mul_rhs))))) {
      bool all_add = true;

      // verify the reducer computations use add as the root operation
      for (auto* comp : reduce_op1->called_computations()) {
        if (comp->root_instruction()->opcode() != HloOpcode::kAdd) {
          all_add = false;
          break;
        }
      }

      for (auto* comp : reduce_op2->called_computations()) {
        if (comp->root_instruction()->opcode() != HloOpcode::kAdd) {
          all_add = false;
          break;
        }
      ...
      if (reduce_input1 == reduce_input2 && all_add) {  // ensure shared input B and add reducers
        HloInstruction* two_scalar = multiply->parent()->AddInstruction(
            HloInstruction::CreateConstant(LiteralUtil::CreateR0<float>(2.0f)));
        HloInstruction* two_broadcast = multiply->parent()->AddInstruction(
            HloInstruction::CreateBroadcast(reduce_bcast1->shape(), two_scalar, {}));
        HloInstruction* reduce_sq = multiply->parent()->AddInstruction(
            HloInstruction::CreateBinary(reduce_bcast1->shape(), HloOpcode::kPower, reduce_bcast1, two_broadcast));
        HloInstruction* new_left_mul = multiply->parent()->AddInstruction(
            HloInstruction::CreateBinary(multiply->shape(), HloOpcode::kMultiply, mul_lhs, reduce_sq));
        
        // replace with multiply of (A * Square(ReduceSum(B))) and C
        return ReplaceWithNewInstruction(
            multiply,
            HloInstruction::CreateBinary(multiply->shape(), HloOpcode::kMultiply, new_left_mul, mul_rhs));
      }
    }
  }

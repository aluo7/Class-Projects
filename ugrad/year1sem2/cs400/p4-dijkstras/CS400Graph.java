...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            if (copyPath.dataSequence.size()!=0){}

               // List<T> deep=new LinkedList<T>();
               this.dataSequence = new LinkedList<>();
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        ...
        //if not in there or null, throw no such element exc.

        if (first==null||last==null||start==null||end==null){
            throw new NoSuchElementException("no vertex for start or end can be found");
        }

        //start at the start node, and if it equals the end you already have the shortest path
        if (start==end){
        ...
        Path shortestPath=new Path(new Vertex(start));

        Vertex curr=first;
        Path prepoll=null;

        //check if this node is the end and if not, follow all the edges leaving your current node and create new paths
        while (!curr.data.equals(end)){
            //shortP.clear();
            //check for destination nodes from edges
            for (Edge e: curr.edgesLeaving){
                if (e.target!=null)
                    paths.add(new Path(shortestPath, e));
            }
            visited.add(curr);
            prepoll=paths.peek();
            ...
            if (paths.size()!=0){
                ...
                //making sure no vertex repetition
                while (visited.contains(curr)){
                    prepoll=paths.peek();
                    curr=paths.poll().end;
                }
                shortestPath=prepoll;
            }
            else{
                throw new NoSuchElementException("There is no path found from start to end.");
            }
        }
